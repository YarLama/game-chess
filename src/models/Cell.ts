...
export class Cell {
    ...
    public isEmptyVertical(target: Cell): boolean {
        ...
        for(let y = min + 1; y < max; y++) {
            if(!this.board.getCell(this.x, y)._isEmpty()) return false;
        }
        return true;
    }
    ...
    public isEmptyHorizontal(target: Cell): boolean {
        ...
        for(let x = min + 1; x < max; x++) {
            if(!this.board.getCell(x, this.y)._isEmpty()) return false;
        }
        return true;
    }
    ...
    public isEmptyDiagonal(target: Cell): boolean {
        ...
        for(let i = 1; i < absY; i++) {
            let x = this.x + dX * i;
            let y = this.y + dY * i;
            if(!this.board.getCell(x, y)._isEmpty()) return false;
        }
